<div>
	...
	<div class="flex mb-5 space-x-5">
		...
		<div>
			...
			<x-select name="sort" wire:model="state.sort.value">
				@foreach($state['sort']['options'] as $key => $value)
		...
		<div>
			...
			<x-select name="group" wire:model="state.group.value">
				@foreach($state['group']['options'] as $value)
	...
	@if($count < $total)
		<x-secondary-button class="justify-center mt-5 text-base w-full" wire:click="showNextMonths" wire:target="showNextMonths" wire:loading.attr="disabled">
			<span wire:loading wire:target="showNextMonths"><em class="fas fa-circle-notch fa-spin"></em></span>
			<span wire:loading.remove wire:target="showNextMonths">{{ __('member/show.links.load-more') }}</span>
		</x-secondary-button>
	@endif
